...
generated `__main__.py` uses a 3-line try-import that falls back to a
**vendored** copy in `._vendor.rpc_server` — same protocol, no external
dependency. The vendored module is regenerated by `stitch_plugin_tools
dev-install` / `vendor` / `publish`. See [docs/service-plugins.md §3](service-plugins.md#3-using-rpcpluginserver)
for the server-side usage pattern and the vendoring model.
...
### 7.8 Vendored RpcPluginServer — standalone operation

Service plugins run standalone in production (spawned with
`cwd=package_dir` and no `autoreg` on `sys.path`). The generated
`__main__.py` uses a 3-line try-import that falls back to a vendored
copy of `RpcPluginServer`:
...
the canonical `autoreg/plugin/rpc.py` by `stitch_plugin_tools` at
scaffold, dev-install, and publish time. It contains the server-side
subset only (`RpcPluginServer`, exception classes, `_error` helper) —
no client-side code. The file is stdlib-only and importable standalone.

**Convention:** never hand-edit `_vendor/rpc_server.py` — it is
regenerated from canonical on every `dev-install` / `vendor` / publish.
The `upgrade` tool byte-compares it against canonical and swaps on
drift. Authors do not need to create or maintain the `_vendor/`
directory — the tooling handles it.
