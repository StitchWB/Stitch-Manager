...
'./profile-settings';
import { Button, ConfirmDialog, IconButton, Input, TabButton } from '@/components/ui';
        ...
        <aside className="absolute right-0 top-0 h-full w-full max-w-[560px] border-l border-white/10 bg-[#0f121e] shadow-2xl flex flex-col">
          <header className="px-5 py-4 border-b border-white/10 flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              null}
              <div className="mt-1 text-xs text-slate-500">{t("profiles.profile_settings_modal.proxy")}{vm.summary.proxyState}{t("profiles.profile_settings_modal.locale")}{vm.summary.locale}{t("profiles.profile_settings_modal.timezone")}{vm.summary.timezone}{t("profiles.profile_settings_modal.window")}{vm.summary.windowSizeHint}</div>
            </div>
            <IconButton
          ...
          <div className="px-5 py-3 border-b border-white/10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="flex items-end justify-start md:justify-end gap-2">
                <Button size="sm" variant="secondary" leftIcon={<CopyPlus size={14} />} onClick={() => void vm.handleDuplicateProfile()} disabled={vm.loading || vm.saving || vm.duplicating || vm.deleting || vm.exportingBundle || vm.importingBundle || Boolean(vm.aliasValidationError)} isLoading={vm.duplicating}>{t("profiles.profile_settings_modal.duplicate")}</Button>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Button size="xs" variant="secondary" leftIcon={<Download size={12} />} onClick={() => void vm.handleExportProfile()} disabled={vm.loading || vm.saving || vm.exportingBundle} isLoading={vm.exportingBundle}>{t("profiles.profile_settings_modal.export")}</Button>
              <Button size="xs" variant="secondary" leftIcon={<FileUp size={12} />} onClick={() => void vm.handleImportProfile()} disabled={vm.loading || vm.saving || vm.importingBundle} isLoading={vm.importingBundle}>{t("profiles.profile_settings_modal.import")}</Button>
              <div className="h-4 w-px bg-white/10 mx-1" />
              <Button size="xs" variant="secondary" leftIcon={<RotateCcw size={12} />} onClick={vm.handleResetCurrentTab} disabled={vm.loading || vm.saving}>{t("profiles.profile_settings_modal.reset_tab")}</Button>
              <Button size="xs" variant="secondary" onClick={() => vm.setResetAllConfirmOpen(true)} disabled={vm.loading || vm.saving}>{t("profiles.profile_settings_modal.reset_all")}</Button>
            </div>
          </div>
