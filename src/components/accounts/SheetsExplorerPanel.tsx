import { t } from "@/lib/i18n";import { useMemo, useState } from 'react';
  ...
  return (
    <div className={cn('flex flex-col h-full overflow-hidden', className)}>
      <div className="shrink-0 border-b border-white/5 bg-ds-surface-base/60 px-4 py-3">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 text-xs text-slate-400">
            ...
            <span className="font-semibold text-white">{t("accounts.sheets_explorer_panel.sheets_explorer")}</span>
            <span className="text-slate-500">•</span>
            <span className="tabular-nums">{svcSheets.length}{t("accounts.sheets_explorer_panel.svc_sheets")}</span>
          </div>
      ...
      <div className="flex-1 overflow-auto px-4 py-4 space-y-4">
        {isLoading ?
        <div className="rounded-xl border border-white/10 bg-ds-surface-overlay/70 p-6 text-sm text-slate-400">{t("accounts.sheets_explorer_panel.loading_sheets_preview")}
        ...
        <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-200 flex items-center justify-between">
          ...
          <ButtonBase
            type="button"
            onClick={onRetry}
            className="text-xs font-semibold text-rose-200 hover:text-white">{t("accounts.sheets_explorer_panel.retry")}
        ...
        <div className="rounded-xl border border-white/10 bg-ds-surface-overlay/80 overflow-hidden">
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
              <div>
                ...
                <div className="text-[11px] text-slate-500">
                  {selectedSheet.rowCount ?? selectedSheet.rows.length}{t("accounts.sheets_explorer_panel.rows")}
              </div>
              </div>
            ...
            <UITable>
              <TableHeader>
                ...
                <TableHead key={col.label}>{col.label}</TableHead>
                  ...
                  <TableHead>{t("accounts.sheets_explorer_panel.actions")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                ...
                <TableCell key={col.label}>
                  ...
                  <span className="text-slate-500">—</span>
                    ...
                    <TableCell>
                      ...
                      }}>{t("accounts.sheets_explorer_panel.show_in_graph")}
                    ...
                    <ButtonBase
                      ...
                      }}>{t("accounts.sheets_explorer_panel.open_sheet")}
            ...
            <div className="px-4 py-3 text-[11px] text-slate-500 border-t border-white/5">{t("accounts.sheets_explorer_panel.showing")}
            {Math.min(filteredRows.length, 20)}{t("accounts.sheets_explorer_panel.of")}{filteredRows.length}{t("accounts.sheets_explorer_panel.rows")}
          </div>
          </div> :
        null}
      </div>
