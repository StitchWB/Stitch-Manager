...
import { Button, GlassCard, Badge, IconButton, Input } from '@/components/ui';
import { registrationControl } from '../../lib/tauri';
import type { PipelineStepOverride } from './PipelineStepConfigPanel';
import { loadEnabledOverrides, saveEnabledOverrides, loadPauseOverrides, savePauseOverrides } from './PipelineStepConfigPanel';
  ...
  return (
    <GlassCard className="shrink-0 border-indigo-500/20 bg-gradient-to-b from-indigo-500/[0.06] to-transparent">
      ...
      <div className="border-t border-white/5">
          ...
          className="w-full flex items-center justify-between px-3 py-1.5 hover:bg-white/[0.02] transition-colors cursor-pointer"
          role="button"
          tabIndex={0}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setShowStepConfig(!showStepConfig); }}}>
        ...
        <div className="px-3 pb-2 space-y-0.5">
          ...
          <div key={step.id} className="flex items-center gap-2 py-0.5">
              ...
              className={cn(
                ...
                variant="ghost"
                onClick={() => {
                const next = pipelineSteps.map((s) =>
                  s.id === step.id ? { ...s, enabled: !s.enabled } : s
                );
                const toggled = next.find(s => s.id === step.id);
                if (toggled) {
                  const overrides = loadEnabledOverrides();
                  overrides[step.id] = toggled.enabled;
                ...
                if (toggled) {
                  ...
                  overrides[step.id] = toggled.pauseAfter;
      ...
      {isPythonBlocked &&
      <div className="px-3 pb-3 text-xs text-slate-500">{t("autoReg.launch_pad.python_drissionpage")}
