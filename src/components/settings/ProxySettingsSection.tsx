        ...
        <>
            ...
            <div className="space-y-2">
              <div className="text-sm text-slate-400" role="presentation">
                {t('settings.proxySettingsSection.proxyTypeLabel')}
              </div>
              <div className="flex gap-2">
                ...
                'bg-slate-800 text-slate-400 hover:bg-slate-700'}
                  `
                }>{t("settings.proxy_settings_section.http")}
                ...
                'bg-slate-800 text-slate-400 hover:bg-slate-700'}
                  `
                }>{t("settings.proxy_settings_section.socks5")}
            ...
            <div className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-slate-300">{t('settings.proxySettingsSection.useProxyListLabel')}</span>
              </div>
              <Toggle
              label=""
          ...
          <>
                <Input
              type="text"
              label={t('settings.proxySettingsSection.proxyUrlLabel')}
              value={proxyUrl}
              onChange={(e) => handleProxyChange(e.target.value)}
              onBlur={(e) => onValidate(e.target.value)}
              placeholder={t('settings.proxySettingsSection.proxyUrlPlaceholder')}
            ...
            <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3 space-y-2">
                    <div className="flex items-center gap-2 text-green-400 text-sm font-medium">
                      ...
                      <span>{t('settings.proxySettingsSection.proxyRecognizedSuccess')}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-slate-800/50 rounded p-2">
                        <div className="text-slate-400 mb-1">{t('settings.proxySettingsSection.hostLabel')}</div>
                        <div className="text-slate-200 font-mono">{parsedProxy.host}</div>
                      </div>
                      <div className="bg-slate-800/50 rounded p-2">
                        <div className="text-slate-400 mb-1">{t('settings.proxySettingsSection.portLabel')}</div>
                        <div className="text-slate-200 font-mono">{parsedProxy.port}</div>
                      </div>
                      {parsedProxy.username &&
                <>
                          <div className="bg-slate-800/50 rounded p-2">
                            <div className="text-slate-400 mb-1">{t('settings.proxySettingsSection.loginLabel')}</div>
                            <div className="text-slate-200 font-mono">{parsedProxy.username}</div>
                          </div>
                          <div className="bg-slate-800/50 rounded p-2">
                            <div className="text-slate-400 mb-1">{t('settings.proxySettingsSection.passwordLabel')}</div>
                      ...
                      <div className="bg-slate-800/50 rounded p-2">
                        <div className="text-slate-400 mb-1">{t('settings.proxySettingsSection.typeLabel')}</div>
                        <div className="text-slate-200 font-mono uppercase">{parsedProxy.type}</div>
                      </div>
                      <div className="bg-slate-800/50 rounded p-2">
                        <div className="text-slate-400 mb-1">{t('settings.proxySettingsSection.statusLabel')}</div>
                        <div className="text-green-400 font-medium">{t('settings.proxySettingsSection.proxyReady')}</div>
                      </div>
                    </div>
                  </div>
            }
            ...
            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                ...
                <div className="flex items-start gap-2 text-xs text-slate-400 bg-slate-800/30 rounded p-3">
                  ...
                  <div className="space-y-1">
                    <p>{t('settings.proxySettingsSection.supportedFormats')}</p>
                    <ul className="list-disc list-inside space-y-0.5 ml-2">
                      <li>
                        <code className="text-slate-300">{'ip:port:username:password'}</code>{' '}-{' '}
                        {t('settings.proxySettingsSection.withAuth')}
                      </li>
                      <li>
                        <code className="text-slate-300">{'ip:port'}</code>{' '}-{' '}
                        {t('settings.proxySettingsSection.withoutAuth')}
                      </li>
                    </ul>
                    <p className="mt-2">
                      {t('settings.proxySettingsSection.example')}{': '}{' '}
