...
import { ArrowLeft, Server, Globe, Search } from 'lucide-react';
...
interface GatewaySectionsProps {
  onPastePackage: () => void;
}

export function GatewaySections({ onPastePackage }: GatewaySectionsProps) {
  ...
  const { fetchUpstreamModels } = useAiGatewayStore();
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-lg p-6">
            ...
            <div className="text-sm text-slate-400 space-y-1">
              <div>
                {t('aiGateway.adapter')}: {view.endpoint.adapterType}
              </div>
              <div>
                {t('aiGateway.baseUrl')}: {view.endpoint.baseUrl}
              </div>
              <div>
                {t('aiGateway.statusLabel')}:{' '}
                {view.endpoint.enabled ? t('aiGateway.enabled') : t('aiGateway.disabled')}
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-lg p-6">
            ...
            <div className="text-sm text-slate-400">
              {t('aiGateway.statusLabel')}:{' '}
              {view.publicModel.enabled ? t('aiGateway.enabled') : t('aiGateway.disabled')}
            </div>
