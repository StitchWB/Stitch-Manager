import { t } from "@/lib/i18n";import type { CompiledFlowPlan } from '@/lib/scenarioFlow/types';
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-md border border-emerald-400/25 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-100 space-y-1">
          <div className="font-medium">{t("recorder.composer_run_tab.last_run_trace")}
          {runTrace.mode ?? 'unknown'})
      ...
      <div className="rounded-md border border-amber-400/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">{t("recorder.composer_run_tab.running_now")}
        {runTrace.currentNodeName ?? runTrace.currentNodeId}
