import { t } from "@/lib/i18n";import ReactFlow, {
  Background,
  type Connection,
  Controls,
  MiniMap,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,1fr)] gap-3">
        <div
          ...
          <div className="rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-slate-300 space-y-3">
              <div className="font-medium text-slate-100">{t("recorder.composer_flow_tab.start_by_adding_your_first_step")}</div>
              <div className="flex flex-wrap gap-2">
                <Button size="sm" variant="secondary" onClick={onAddRunNode}>{t("recorder.composer_flow_tab.add_auth_node")}

              </Button>
                <Button size="sm" variant="secondary" onClick={onAddSwitchNode}>{t("recorder.composer_flow_tab.add_context_node")}

              </Button>
                <Button size="sm" onClick={onCreateStarterTemplate}>{t("recorder.composer_flow_tab.use_starter_template")}
        ...
        <div className="rounded-lg border border-white/10 bg-black/20 p-3 space-y-3 max-h-[460px] overflow-y-auto">
          ...
          <div className="rounded-md border border-rose-400/30 bg-rose-500/10 px-3 py-2 text-xs text-rose-200 flex items-center justify-between gap-2">
              <span>{t("recorder.composer_flow_tab.selected_branch")}{selectedEdgeId}</span>
              <Button size="xs" variant="danger" onClick={onClearSelectedEdgeBranch}>{t("recorder.composer_flow_tab.clear_branch")}
          ...
          <div className="space-y-2">
              <div className="flex flex-wrap gap-2">
                <Button size="xs" variant="secondary" onClick={onSetSelectedAsStart}>{t("recorder.composer_flow_tab.set_as_start")}

              </Button>
                <Button size="xs" variant="secondary" onClick={onDuplicateSelected}>{t("recorder.composer_flow_tab.duplicate")}
