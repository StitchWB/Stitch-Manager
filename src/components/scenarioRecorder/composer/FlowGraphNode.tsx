import { t } from "@/lib/i18n";import { Handle, Position, type NodeProps } from 'reactflow';
  ...
  return (
    <div
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="text-[10px] text-slate-400">{t("recorder.flow_graph_node.step")}{data.index + 1}</div>
      </div>
      ...
      {data.executed ?
      <Badge variant="info" size="sm" className="mt-1 normal-case tracking-normal text-[10px]">{t("recorder.flow_graph_node.executed")}

      </Badge> :
      null}
      {!data.executed && data.running ?
      <Badge variant="warning" size="sm" className="mt-1 normal-case tracking-normal text-[10px]">{t("recorder.flow_graph_node.running")}
