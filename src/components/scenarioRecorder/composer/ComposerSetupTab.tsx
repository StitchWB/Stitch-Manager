import { t } from "@/lib/i18n";import { Button, FormGrid, Input, Select, Textarea } from '@/components/ui';
    ...
    <>
      ...
      <div className="rounded-lg border border-white/10 bg-black/20 p-3 space-y-2">
        <div className="flex items-center justify-between">
          <div className="text-xs text-slate-300">{t("recorder.composer_setup_tab.flow_input_defaults")}</div>
          <Button size="xs" variant="secondary" onClick={addInputDefault}>{t("recorder.composer_setup_tab.add_input")}

          </Button>
        </div>
        {inputDefaultEntries.length === 0 ?
        <div className="text-xs text-slate-500">{t("recorder.composer_setup_tab.no_flow_inputs_yet")}</div> :
          ...
          <FormGrid responsive key={key}>
              ...
              className="h-9" />
                ...
                className="h-9" />
                  ...
                  <Button size="xs" variant="danger" onClick={() => removeInputDefault(key)}>{t("recorder.composer_setup_tab.remove")}
      ...
      <div className="rounded-lg border border-white/10 bg-black/20 p-3 space-y-2">
        <div className="text-xs text-slate-400">{t("recorder.composer_setup_tab.import_emails_from_google_sheets")}</div>
        {!sheetsParams ?
        <div className="text-xs text-amber-300">{t("recorder.composer_setup_tab.configure_google_sheets_credentials_in_autoreg_set")}
        ...
        <FormGrid columns={3} responsive>
            ...
            <div className="flex items-end gap-2">
              <Button variant="secondary" onClick={() => void refreshSheets()}>{t("recorder.composer_setup_tab.refresh_sheets")}

            </Button>
                <Button variant="secondary" onClick={importEmailsFromSheet}>{t("recorder.composer_setup_tab.import")}
