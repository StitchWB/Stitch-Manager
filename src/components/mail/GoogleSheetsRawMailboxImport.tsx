    ...
    <section className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-white">
          <Database size={16} />
          <h2 className="text-sm font-semibold">{t("mail.google_sheets_raw_mailbox_import.google_sheets_raw")}</h2>
        </div>
        <Button
          size="xs"
          variant="ghost"
          ...
          onClick={() => {
            void refresh();
          }}>{t("mail.google_sheets_raw_mailbox_import.refresh")}
      ...
      {!sheetsParams ?
      <p className="text-xs text-slate-500">{t("mail.google_sheets_raw_mailbox_import.configure_google_sheets_credentials_in_settingsreg")}
      ...
      <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-2 space-y-1">
          <p className="text-xs text-amber-100 flex items-center gap-1">
            <AlertTriangle size={12} /> {parsed.issues.length}{t("mail.google_sheets_raw_mailbox_import.row_issues")}
        </p>
      ...
      <div className="space-y-2">
          <p className="text-xs text-slate-400">{t("mail.google_sheets_raw_mailbox_import.detected_mailboxes")}{parsed.drafts.length}</p>
          <Select
          label="Mailbox draft"
