...
function parseCurrentStep(logs: LogEntry[], isRunning: boolean): ProcessStep {
  ...
  for (let i = logs.length - 1; i >= 0; i--) {
    ...
    // Success states
    if (msg.includes('account created') || msg.includes('registration complete') || logs[i].level === 'success') {
      return 'done';
    }
    ...
    // Verification stage
    if (msg.includes('verification') || msg.includes('code found') || msg.includes('entering code') || 
        msg.includes('verify') || msg.includes('allow access')) {
      return 'verify';
    }
    
    // Auth stage (filling forms)
    if (msg.includes('entering') || msg.includes('typing') || msg.includes('password') || 
        msg.includes('email') || msg.includes('signup') || msg.includes('sign up') ||
        msg.includes('create account') || msg.includes('form')) {
      return 'auth';
    }
    
    // Browser stage
    if (msg.includes('browser') || msg.includes('chromium') || msg.includes('headless') ||
        msg.includes('navigat') || msg.includes('opening') || msg.includes('window')) {
      return 'browser';
    }
    
    // IMAP stage
    if (msg.includes('imap') || msg.includes('inbox') || msg.includes('mail') || 
        msg.includes('connected to imap') || msg.includes('gmail')) {
      return 'imap';
    }
...
function parseLiveAction(logs: LogEntry[], isRunning: boolean): { action: LiveAction; detail?: string } {
  ...
  if (msg.includes('entering password') || msg.includes('typing password')) {
    ...
    return { action: 'typing_password', detail: lengthMatch ? `${lengthMatch[1]} characters` : undefined };
  }
  
  if (msg.includes('entering email') || msg.includes('typing email') || (msg.includes('entering') && msg.includes('@'))) {
    return { action: 'typing_email' };
  }
  
  if (msg.includes('entering code') || msg.includes('typing code') || msg.includes('verification code')) {
  ...
  const stats = useMemo(() => {
    ...
    for (const log of logs) {
      ...
      if (msg.toLowerCase().includes('chromium') || msg.toLowerCase().includes('browser')) {
        if (msg.toLowerCase().includes('initialized') || msg.toLowerCase().includes('successfully')) {
...
export function MissionControlHUD({ logs, isRunning = false, canStart = true, onStart, onClear, className }: MissionControlHUDProps) {
  ...
  return (
    <div className={cn('flex flex-col h-full', className)}>
      {/* Timeline - Top */}
      <div className="shrink-0 border-b border-white/5" style={{ background: 'rgba(0, 0, 0, 0.3)' }}>
