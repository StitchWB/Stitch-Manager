...
def _mint_assertion(role: str, secret: str, sub: str | None = None) -> str:
    ...
    without a second login.  ``sub`` (the caller's Telegram id) is included
    when known — the radar contract recommends it for auditability; extra
    claims are ignored by verifiers that don't check them.
    """
    payload_dict: dict[str, Any] = {"role": role, "exp": int(time.time()) + 300}
    if sub:
...
def _auth_headers(role: str | None = None, sub: str | None = None) -> dict[str, str]:
    s = get_settings()
    if s.radar_shared_secret:
        return {
            "Authorization": f"Bearer {_mint_assertion(role or '', s.radar_shared_secret, sub)}"
        }
...
async def _get(path: str, params: dict[str, str] | None = None,
               role: str | None = None, sub: str | None = None) -> dict:
    async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
        try:
            resp = await client.get(
                f"{_keys_base_url()}{path}",
                params=params,
                headers=_auth_headers(role, sub),
            )
...
async def fetch_found_keys(
    params: FoundKeysParams, role: str | None = None, sub: str | None = None
) -> dict:
    """Proxy ``GET /api/found-keys`` (masked list, VIP+ via assertion)."""
    return await _get("/api/found-keys", params.to_query(), role, sub)


async def fetch_found_key_secret(
    key_id: int, role: str | None = None, sub: str | None = None
) -> dict:
    """Proxy ``GET /api/found-keys/{id}/secret`` (VDS-only, VIP+)."""
    return await _get(f"/api/found-keys/{key_id}/secret", role=role, sub=sub)
